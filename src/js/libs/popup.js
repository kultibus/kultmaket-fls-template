...
class Popup {
			...
			if (this.targetOpen.element) {
				...
				if (this.youTubeCode) {
					...
					if (!this.targetOpen.element.querySelector(`[${this.options.youtubePlaceAttribute}]`)) {
						const youtubePlace = this.targetOpen.element.querySelector('.popup__body').setAttribute(`${this.options.youtubePlaceAttribute}`, '');
					}
	...
	close(selectorValue) {
		...
		if (!this._reopen) {
			...
			if (!this.bodyLock && !document.documentElement.classList.contains('menu-open')) {
				bodyUnlock();
			}
			this.isOpen = false;
		}
